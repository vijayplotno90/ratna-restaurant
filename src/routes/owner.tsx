...
function Dashboard({ data, tab, setTab, busy, error, onRefresh, onSignOut }: { data: OwnerData | null; tab: Tab; setTab: (tab: Tab) => void; busy: boolean; error: string; onRefresh: () => Promise<void>; onSignOut: () => void }) {
  ...
  return <>
    ...
    {tab === "pnl" && <PnLTab model={model} leads={data?.leads ?? []} />}{tab === "approvals" && <ApprovalsTab approvals={data?.approvals ?? []} />}{tab === "customers" && <CustomersTab model={model} />}
    {tab === "messages" && <MessagesTab messages={data?.messages ?? []} deliveries={data?.deliveries ?? []} />}{tab === "automations" && <AutomationsTab campaigns={data?.campaigns ?? []} />}
  </>;
}
...
function PnLTab({ model, leads }: { model: ReturnType<typeof buildModel>; leads: any[] }) { const labels = ["Today", "Last 7 days", "Last 30 days", "Year to date"]; const maxSeries = Math.max(1, ...model.daySeries.map(point => point.revenue)); const maxHour = Math.max(1, ...model.hours.map(hour => hour.orders)); const last30 = leads.filter(lead => Date.now() - new Date(lead.created_at).getTime() <= 30 * 86400000); const whatsapp = last30.filter(lead => lead.lead_type === "whatsapp").length; const phone = last30.filter(lead => lead.lead_type === "phone").length; return <section className="mt-6 space-y-6">
  ...
  <div className="grid gap-5 lg:grid-cols-2"><div className="owner-card"><h2 className="font-serif text-xl italic text-[var(--emerald-deep)]">Daily revenue & net profit</h2><p className="mt-1 text-xs text-muted-foreground">Last 30 days · hover-free visual snapshot</p><div className="mt-5 flex h-44 items-end gap-[3px] overflow-hidden">{model.daySeries.map(point => <div key={point.label} title={`${point.label}: ${money(point.revenue)}`} className="min-w-[4px] flex-1 rounded-t bg-[var(--emerald)]/80" style={{ height: `${Math.max(3, point.revenue / maxSeries * 100)}%` }} />)}</div><div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>{model.daySeries[0]?.label}</span><span>Today</span></div></div><div className="owner-card"><p className="eyebrow">Website-generated leads</p><h2 className="mt-1 font-serif text-xl italic text-[var(--emerald-deep)]">WhatsApp and phone intent</h2><p className="mt-1 text-xs text-muted-foreground">Every tap is attributed to the website placement.</p><div className="mt-4 grid grid-cols-3 gap-2"><Stat label="30 days" value={String(last30.length)} /><Stat label="WhatsApp" value={String(whatsapp)} /><Stat label="Calls" value={String(phone)} /></div><p className="mt-3 text-xs text-muted-foreground">Top sources: {Object.entries(last30.reduce((all: Record<string, number>, lead: any) => ({ ...all, [lead.placement]: (all[lead.placement] ?? 0) + 1 }), {})).slice(0, 3).map(([name, count]) => `${name.replaceAll("_", " ")} (${count})`).join(" · ") || "No leads recorded yet"}</p></div></div>
  <div className="grid gap-5 lg:grid-cols-2"><div className="owner-card"><h2 className="font-serif text-xl italic text-[var(--emerald-deep)]">Peak order hours</h2><p className="mt-1 text-xs text-muted-foreground">When the kitchen receives orders</p><div className="mt-5 flex h-32 items-end gap-1">{model.hours.map(hour => <div key={hour.hour} title={`${String(hour.hour).padStart(2, "0")}:00 · ${hour.orders} orders`} className="flex-1 rounded-t bg-[var(--brass)]/80" style={{ height: `${Math.max(3, hour.orders / maxHour * 100)}%` }} />)}</div><div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>00</span><span>06</span><span>12</span><span>18</span><span>23</span></div></div><div className="owner-card"><p className="eyebrow">Tax-ready operations</p><h2 className="mt-2 font-serif text-xl italic text-[var(--emerald-deep)]">Clean records for the CA</h2><p className="mt-2 text-sm text-muted-foreground">All Ratna orders retain sales and GST values. The finance export will group invoices by period for a single CA download.</p><p className="mt-5 text-3xl font-serif italic text-[var(--emerald-deep)]">{money(model.totals[3]?.revenue ?? 0)}</p><p className="text-xs uppercase tracking-wider text-muted-foreground">Recorded sales</p></div></div>
...
function ApprovalsTab({ approvals }: { approvals: any[] }) { const pending = approvals.filter(item => item.status === "pending"); return <section className="mt-6 owner-card"><p className="eyebrow">Owner approval protects the live menu</p><h2 className="mt-2 font-serif text-2xl italic text-[var(--emerald-deep)]">Approvals</h2><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Every admin proposal—dish, offer, special, custom dish or website update—comes here first. Nothing goes live until the owner decides.</p>{pending.length ? <div className="mt-6 space-y-4">{pending.map(request => <article key={request.id} className="rounded-sm border border-[var(--brass)]/25 p-5"><div className="flex flex-wrap justify-between gap-3"><div><p className="eyebrow">{request.change_type}</p><h3 className="mt-1 font-serif text-xl italic text-[var(--emerald-deep)]">{request.target_name}</h3><p className="mt-2 text-sm text-muted-foreground">{request.summary}</p></div><span className="h-fit rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold uppercase text-amber-800">Pending</span></div><p className="mt-3 text-xs text-muted-foreground">Requested by {request.requested_by} · {dateTime(request.requested_at)}</p><div className="mt-4 flex gap-2"><button disabled className="pill-action opacity-50">Approve</button><button disabled className="pill-action opacity-50">Request changes</button></div></article>)}</div> : <div className="mt-6 flex items-center gap-3 rounded-sm bg-[var(--ivory)] p-5"><CheckCircle2 className="h-6 w-6 text-[var(--emerald)]" /><p className="text-sm"><b>No pending approvals.</b><br /><span className="text-muted-foreground">The admin workspace is ready for the next submitted change.</span></p></div>}<p className="mt-4 text-xs text-muted-foreground">Approval actions are being wired next; the queue already uses Ratna’s live Supabase records.</p></section>; }
